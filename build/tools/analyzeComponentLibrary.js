...
function extractReactProps(content) {
    const props = [];
...
function extractVueProps(content) {
    const props = [];
...
function extractAngularProps(content) {
    const props = [];
...
function extractComponentStyles(filePath, content, framework) {
    const styles = [];
...
function analyzeComponentUsage(component) {
    ...
    if (props.some(p => p.name === 'children'))
        designPatterns.push('composition');
    if (props.some(p => p.name.includes('onClick') || p.name.includes('onPress')))
        designPatterns.push('interactive');
    if (props.some(p => p.name.includes('variant') || p.name.includes('size')))
        designPatterns.push('variants');
    if (props.some(p => p.name.includes('disabled') || p.name.includes('loading')))
        designPatterns.push('states');
...
async function analyzeComponentLibraryHandler(args) {
    ...
    allFiles = allFiles.filter(file => {
        ...
        return (basename[0] === basename[0].toUpperCase() || // PascalCase
    ...
    const frameworkDeps = [...allDependencies].filter(dep => ['react', 'vue', '@angular', 'svelte'].some(framework => dep.includes(framework)));
    const stylingDeps = [...allDependencies].filter(dep => ['styled-components', '@emotion', 'sass', 'less', 'tailwind'].some(style => dep.includes(style)));
    const utilityDeps = [...allDependencies].filter(dep => !frameworkDeps.includes(dep) && !stylingDeps.includes(dep));
...
export const analyzeComponentLibraryTool = {
...
//# sourceMappingURL=analyzeComponentLibrary.js.map