...
async function extractFromCSS(filePath) {
            ...
            if (node.type === 'Rule') {
                ...
                if (rule.prelude && rule.prelude.type === 'SelectorList') {
                    ...
                    if (selector.includes(':root') || selector.includes('[data-theme')) {
                        csstree.walk(rule.block, (declNode) => {
...
async function extractFromSCSS(filePath) {
    const content = await fs.readFile(filePath, 'utf-8');
    const tokens = [];
...
async function extractFromJSON(filePath) {
    ...
    const tokens = [];
    function processObject(obj, prefix = '', category = '') {
...
function transformTokenName(name, transform) {
    switch (transform) {
        case 'kebab-case':
...
async function extractDesignTokensHandler(args) {
...
//# sourceMappingURL=extractDesignTokens.js.map