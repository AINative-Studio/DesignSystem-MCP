...
class DesignSystemServer {
    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            if (!tool) {
                throw new McpError(ErrorCode.MethodNotFound, `Tool "${name}" not found. Available tools: ${Object.keys(designSystemTools).join(', ')}`);
            }
            try {
                // Validate input
                const validationResult = validateToolInput(name, args || {});
                if (!validationResult.isValid) {
                    throw new McpError(ErrorCode.InvalidParams, `Invalid parameters: ${validationResult.errors.join(', ')}`);
                }
                // Execute tool
