...
function generateColorScale(baseColor: string, steps = 10): Record<string, string> {
...
function generateSemanticColors(baseColors: string[]): Record<string, string> {
...
function checkAccessibility(color1: string, color2: string, targetRatio = 4.5): { ratio: number; compliant: boolean } {
  try {
...
function generateTailwindTheme(colors: Record<string, any>, options: GenerateThemeInput): any {
  return {
    colors: {
      ...colors,
...
function generateStyledComponentsTheme(colors: Record<string, any>, options: GenerateThemeInput): any {
  return {
    colors,
    typography: {
      fontFamily: {
...
function generateMaterialUITheme(colors: Record<string, any>, options: GenerateThemeInput): any {
...
function generateCSSVariables(colors: Record<string, any>, options: GenerateThemeInput): string {
  let css = ':root {\\n';
  
  // Generate color variables
  function addColorVariables(colorObj: any, prefix = 'color') {
...
async function generateThemeHandler(args: GenerateThemeInput): Promise<ThemeResult> {
  ...
  const accessibilitySuggestions: string[] = [];
  ...
  if (accessibility) {
    ...
    if (includeSemanticColors && colors.semantic) {
      for (const [name, color] of Object.entries(colors.semantic)) {
        const contrast = checkAccessibility(color as string, '#ffffff', contrastRatio);
...
export const generateThemeTool: DesignSystemTool = {
  name: 'generateTheme',
